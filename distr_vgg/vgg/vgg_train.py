...
def _tower_loss(images, labels, num_classes, scope, reuse_variables=None):
  ...
  with tf.variable_scope(tf.get_variable_scope(), reuse=reuse_variables):
    logits,endpoints  = vgg.inference(images, num_classes, for_training=True,
                                 restore_logits=restore_logits,
                                 scope=scope)
  print('The shape of logits is {}'.format(logits.get_shape()))
  print(labels)
  print('The shape of the labels is {}'.format(labels.get_shape()))
    ...
    for i in xrange(FLAGS.num_gpus):
      with tf.device('/gpu:%d' % i):
        with tf.name_scope('%s_%d' % (vgg.TOWER_NAME, i)) as scope:
          ...
          #Should the model variables also be on the CPU? slim.model_variable
