...
class VGG19Test(tf.test.TestCase):
  ...
  def testBuild(self):
    print('testing build for vgg 19')
    batch_size = 5
    height, width = 224, 224
    num_classes = 1000
    with self.test_session():
  ...
  def testForward(self):
    ...
    with self.test_session() as sess:
      inputs = tf.random_uniform((batch_size, height, width, 3))
      print(inputs)
      print('input tensor shape is {}'.format(inputs.get_shape()))
      logits, _ = vgg.vgg_19(inputs, num_classes=100)
      sess.run(tf.initialize_all_variables())
      output = sess.run(logits)
      print('output for vgg19 {}'.format(logits.get_shape()))
      self.assertTrue(output.any())
